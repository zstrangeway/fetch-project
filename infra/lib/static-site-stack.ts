...
export class StaticSiteStack extends cdk.Stack {
  constructor(parent: Construct, name: string, props: StaticSiteStackProps) {
    ...
    rootBucket.addToResourcePolicy(new iam.PolicyStatement({
    ...
    const distribution = new cfn.Distribution(this, `${name}-Distribution`, {
      ...
      defaultBehavior: {
        origin: new origins.S3Origin(rootBucket, { originAccessIdentity: cloudfrontOAI }),
        compress: true,
    ...
    // Tagging
    [logBucket, rootBucket, cloudfrontOAI, distribution]
      .forEach((construct) => addMetaTags(construct, props.appName, props.service, props.stage));
